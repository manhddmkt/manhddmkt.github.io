(() => {
  ...
  const DEFAULT_TITLE={vi:"OWNEX COMMERCE",en:"OWNEX COMMERCE"};
  let config;
  ...
  const normalizeBrowserTitle=(value={})=>{
    if(typeof value==="string"&&value.trim())return{vi:value.trim(),en:value.trim()};
    return{vi:value?.vi||value?.en||DEFAULT_TITLE.vi,en:value?.en||value?.vi||DEFAULT_TITLE.en};
  };
  ...
  function apply(){
    ...
    const browserTitle=local(config.browserTitle||DEFAULT_TITLE,lang);if(browserTitle)document.title=browserTitle;
  ...
  async function load(){try{const r=await fetch(`${API}/api/public/bootstrap?globals=${Date.now()}`,{cache:"no-store"});if(!r.ok)throw new Error(`Bootstrap ${r.status}`);const p=await r.json();const globals=p?.content?.homepage?.pageBuilder?.globals||{};const site=p?.content?.site||{};const siteLogo=site.logo||{src:site.logoUrl,alt:site.logoAlt,width:site.logoWidth,href:site.logoHref};const siteFavicon=site.favicon||{src:site.faviconUrl,alt:site.faviconAlt};const siteTitle=site.browserTitle||{vi:site.browserTitleVi||site.siteTitle||DEFAULT_TITLE.vi,en:site.browserTitleEn||site.siteTitle||DEFAULT_TITLE.en};config={...globals,browserTitle:normalizeBrowserTitle(siteTitle),favicon:normalizeFavicon(siteFavicon),header:{...(globals.header||{}),logo:{...normalizeLogo(globals.header?.logo||{}),...normalizeLogo(siteLogo)}}};apply();document.documentElement.dataset.ownexGlobalsLoaded="true"}catch(e){document.documentElement.dataset.ownexGlobalsLoaded="error";applyFavicon({src:DEFAULT_FAVICON});console.warn("Page Builder globals:",e)}}
